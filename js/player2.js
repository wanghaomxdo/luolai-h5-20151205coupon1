...
app.template.loader.init= function(){
    ...
    var getSource = function(){
        ...
        res.push("images/p4/m-1.png");
        res.push("images/p1/bg.png");
        res.push("images/p4/e-1.png");
        res.push("images/p4/e-2.png");
        res.push("images/p4/e-3.png");
        res.push("images/p4/e-4.png");
        res.push("images/p4/m-1.png");
        res.push("images/p2/confirm.png");
...
app.p4.init = function(){
    if(app.template.data.add("replay") != "replay")
        this.add_player2();
};

app.p4.bind_touch_event = function(){
    $(".p4 .confirm").on("touchend", function(){
        $(".p4 .confirm").hide();
        $(".p4 .gamerules").hide();
    });

    $(".p4 .e-2").on("touchend", function(){
        app.p4.updateplayer2_thengogamepage("e-2");
    });

    $(".p4 .e-3").on("touchend", function(){
        app.p4.updateplayer2_thengogamepage("e-3");
    });

    $(".p4 .e-4").on("touchend", function(){
        app.p4.updateplayer2_thengogamepage("e-4");
        
    });

};

app.p4.add_player2 = function(){
    $.post("db/addplayer2.php", {id: app.tools.geturlparam("id")}, function(){});
};

app.p4.updateplayer2_thengogamepage = function(bed){
    app.audio.play_tap();
    app.template.data.add("bed", bed);
    $.post("db/updateplayer2.php", {bed: app.template.data.get("bed")}, function(){});
...
app.p6.bind_touch_event = function(){
    $(".p6 .confirm").on("touchend", function(){
        if(app.template.data.get("page") != "game")
        {
            ...
            $(".p6 .wait-player").show();
            $(".p6 .confirm").hide();
            app.p6.check_player1_isready();
        }
    });
};

app.p6.check_player1_isready_timer = 0;
app.p6.check_player1_isready = function(){
    app.p6.check_player1_isready_timer = setInterval(function(){
        var i = 0;
        $.getJSON("db/getplayer1info.php", {id: app.tools.geturlparam("id")}, function(data){
            if(data[0].player1isready == "1")
            {
                clearInterval(app.p6.check_player1_isready_timer);
                app.p6.updateplayer2toready(1);
                app.template.data.add("props", data[0].player1props);
                app.p6.update_layout();
                
                $(".p6 .overlay").hide();

                setTimeout(function(){app.p6.gamestart();}, 1000)
            }
            else
            {
                if(!app.p6.updateplayer2toread_isset)
                    app.p6.updateplayer2toready(1, "set");
            }
        });
    }, 1000);
};

app.p6.updateplayer2toread_isset = false;
app.p6.updateplayer2toready = function(isready, set){
    $.post("db/updateplayer2toready.php", {isready: isready}, function(){
        if(set)
            app.p6.updateplayer2toread_isset = true;    
    });
};
...
app.p6.show_layout = function(){
    $.getJSON("db/getplayer1info.php", {id: app.tools.geturlparam("id")}, function(data){
        var sex = data[0].player1sex == "boy" ? "girl" : "boy";
        var props = data[0].player1props == "" ? "e-2" : data[0].player1props;

        app.template.data.add("sex", sex);
        app.template.data.add("props", props);

        $(".p6 ." + sex).css("display", "block");
        $(".props-" + sex).attr("src", "images/p6/e-2-" + props + ".png");
    });   
};

app.p6.update_layout = function(){
    var sex = app.template.data.get("sex");
    var props = app.template.data.get("props");
    $(".props-" + sex).attr("src", "images/p6/e-2-" + props + ".png");
};
...
app.p6.listener_updatescore = function(){
    app.p6.shaketimer.updatescore_timeout = setTimeout(function(){
        app.p6.shaketimer.updatescore_interval = setInterval(function(){
            $.getJSON('db/updatescorebyplayer2.php', {shaketime: app.p6.shaketime, score: app.p6.shaketimercountdown}, function(data){
                var player1shaketime = parseInt(data[0].player1shaketime);
                var player2shaketime = parseInt(data[0].player2shaketime);
                //$("#shaketime").html("player2in:player1shaketime:" + player1shaketime + ",player2shaketime:" + player2shaketime);
                if((player1shaketime + player2shaketime) > 10 && player1shaketime > 0 && player2shaketime > 0)
                {
                    app.p6.game_win();
                }else if(player2shaketime > 3)
                {
...
app.p6.updateplayer2shaketime = function(){
    $.getJSON('db/updatescorebyplayer2.php', {shaketime: 0, score: 0}, function(){});
};
...
fuckandroid.app.p4 = function(){};
fuckandroid.app.p4.bind_touch_event = function(){
    $(".p4 .confirm").on("touchend", function(){
...
(function(){
    ...
    if(app.tools.platform.debug == "android"
     || app.tools.platform.os == "android")
    {
        app.p4.bind_touch_event = fuckandroid.app.p4.bind_touch_event;
    }
